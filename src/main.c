 ...
 *         - Отрицательные значения при ошибках (см. ReturnCodes)
 * 
 * \warning Ограничения:
...
int main(int argc, char *argv[])
{
    ...
    char str[STR_LEN + 2];          ///< Буфер для входящей строки (+2 для \n и \0)
    char new_str[STR_LEN + 1];      ///< Буфер для результирующей строки
    int line_number = 0;            ///< Счетчик обработанных строк
    memset(str, '\0', sizeof(str));
    memset(new_str, '\0', sizeof(new_str));

    char *words_array[WORD_NUMBER]; ///< Массив указателей на слова
